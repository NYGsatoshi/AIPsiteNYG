...
@Component({
  ...
  template: `
    <section
      class="saved"
      aria-labelledby="saved-messages-heading"
      data-testid="saved-messages-page"
    >
      ...
      @if (facade.view().status === 'loading') {
          ...
          @for (item of facade.view().items; track item.messageId) {
            <li class="saved__item" data-testid="saved-message-item">
              <div class="saved__meta">
                ...
                <span
                  >{{ item.authorDisplayName }} · {{ item.messageCreatedAt | date: 'medium' }}</span
                >
              </div>
              <p>{{ item.body }}</p>
        ...
        <nav class="saved__paging" aria-label="Saved message pages">
          <button
            type="button"
            [disabled]="facade.view().page <= 1"
            (click)="facade.load(facade.view().page - 1)"
          >
            Previous
          </button>
