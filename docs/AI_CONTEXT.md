...
| Announcements | Partially implemented; Issue #378 immediate-publish confirmation candidate | The server-authorized audience projection returns only an authorized scope, display name, and estimated recipient count; it does not disclose recipient identities. The Angular adapter maps its numeric priority and scope IDs to the existing create DTO. The #378 candidate adds an explicit immediate-publication review before the existing POST, then preserves the editable current-tab form after a generic failure or a stale selected-audience denial. The controller reauthorizes only that selected scope immediately before creation; browser review state is not authorization. This is deliberately non-closing: there is no scheduling or timezone picker, persistent/saved draft, content preview, idempotency/replay, or scheduler contract, and real-backend confirmation remains pending. See `docs/verification/p0-announcement-publish-confirmation.md`. |
