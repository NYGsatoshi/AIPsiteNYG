...
| Local filesystem files | Partially implemented; Issue #345 contextual actions verified | Authorization, policy, repository, and storage exist. The Workspace inventory is a bounded server-authorized list and now projects a per-row `canDelete` presentation capability without per-row authorization queries. The Angular Files surface replaces its normal toolbar with selected-row actions, confirms destructive targets by the authorized server-projected display label/count, and processes multi-selection through the existing single-file DELETE route serially with explicit partial/non-atomic reporting. Canonically redacted File metadata stays redacted through confirmation. `canDelete` is advisory only: every DELETE re-runs the complete current owner-scope policy. Rename, move, share, public links, and an atomic batch-delete contract remain absent. Upload/database failure cleanup and controlled missing-file handling are still incomplete. See `docs/verification/p0-files-contextual-actions.md`. |
