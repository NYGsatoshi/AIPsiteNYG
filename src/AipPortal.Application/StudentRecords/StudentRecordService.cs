    ...
    IAuditLogger auditLogger) : IStudentRecordService
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        {
            await AuditExportDenialAsync(userId, record, null, "request", ReasonDenialCode(request.Reason), requestedFields, null, cancellationToken);
            return Result<StudentRecordExportGrantResponse>.Failure("A valid export reason is required.");
        }

        var decision = await ReauthorizeExportAsync(userId, record, requestedFields, null, "request", cancellationToken);
        if (!decision.IsAllowed)
        {
            await AuditExportDenialAsync(userId, record, null, "request", decision.DenialReason, requestedFields, decision.Access, cancellationToken);
        ...
        await auditLogger.LogAsync(new AuditLogEntry(
            userId,
            "student_record.export.request",
            "ExportPackageGrant",
            grant.Id,
            "Student record restricted export requested.",
            WorkspaceId: record.WorkspaceId,
            Metadata: ExportAuditMetadata(record, grant, "request", "allow", "fresh-reauthorization", decision),
        ...
        CancellationToken cancellationToken)
    {
        ...
        {
            await AuditExportDenialAsync(userId, record, grant, stage, "export-package-grant-expired", SplitFields(grant.RequestedFields), null, cancellationToken);
            return ExportGrantValidation.Failure("Student record export not found.");
        }

        if (requireBuilt && !grant.BuiltAt.HasValue)
        {
            await AuditExportDenialAsync(userId, record, grant, stage, "export-package-not-built", SplitFields(grant.RequestedFields), null, cancellationToken);
            return ExportGrantValidation.Failure("Student record export not found.");
        }
        ...
        CancellationToken cancellationToken)
    {
        ...
        {
            return StudentRecordExportAuthorization.Deny(
                "missing-student-record-restricted-fields",
                requestedRestrictedFields,
                [],
                unknownFieldCount,
                null,
        ...
        if (!requestedRestrictedFields.All(field => authorizedFields.Contains(field, StringComparer.OrdinalIgnoreCase)))
        {
            var reason = existingGrant is not null
                ? stage.Equals("download", StringComparison.OrdinalIgnoreCase)
                    ? "policy-changed-after-build"
                    : "policy-changed-after-request"
                : "unauthorized-restricted-field";
            return StudentRecordExportAuthorization.Deny(
                reason,
                requestedRestrictedFields,
        ...
        CancellationToken cancellationToken)
    {
        await auditLogger.LogAsync(new AuditLogEntry(
            userId,
            $"student_record.export.{stage}",
            "ExportPackageGrant",
            grant.Id,
            $"Student record restricted export {stage} authorized.",
            WorkspaceId: record.WorkspaceId,
            Metadata: ExportAuditMetadata(record, grant, stage, "allow", "fresh-reauthorization", decision),
            TenantId: record.TenantId), cancellationToken);
    }
    ...
    {
        return string.IsNullOrWhiteSpace(reason) ? "missing-reason" : "invalid-reason";
    }
