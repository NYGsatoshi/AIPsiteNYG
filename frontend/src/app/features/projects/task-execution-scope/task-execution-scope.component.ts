...
export class TaskExecutionScopeComponent implements OnChanges, OnDestroy {
  ...
  constructor() {
    this.unregisterProtectedStateClearer =
      this.realtime?.registerProtectedStateClearer(this.owner, () => this.clearProtectedState()) ??
      null;
    this.realtimeEvents =
      this.realtime?.durableEvents$.subscribe((event) => this.handleRealtimeEvent(event)) ?? null;
  }
  ...
  setKindState(target: EditorTarget, kind: SourceKind, event: Event): void {
    ...
    if (kind === 'Web') {
      (target === 'project' ? this.projectWebEnabled : this.overrideWebEnabled).set(
        state !== 'Exclude',
      );
    }
    if (kind === 'ProjectFile') {
      ...
      const enabled =
        state !== 'Exclude' ||
        rules.some((rule) => rule.kind === 'ProjectFile' && rule.state !== 'Exclude');
      (target === 'project' ? this.projectFilesEnabled : this.overrideProjectFilesEnabled).set(
        enabled,
      );
    }
  }
  ...
  removeItemRule(target: EditorTarget, kind: SourceKind, sourceId: string): void {
    const rulesSignal = target === 'project' ? this.projectItemRules : this.overrideItemRules;
    rulesSignal.set(
      rulesSignal().filter((rule) => !(rule.kind === kind && rule.sourceId === sourceId)),
    );
    this.reconcileLegacyFileFlag(target);
  }
  ...
  addSiteRule(target: EditorTarget, value: string): void {
    ...
    const rulesSignal = target === 'project' ? this.projectItemRules : this.overrideItemRules;
    if (rulesSignal().some((rule) => rule.kind === 'WebSite' && rule.sourceId === sourceId)) {
      return;
    }
  ...
  itemState(target: EditorTarget, kind: SourceKind, sourceId: string): SourceState {
    const rules = target === 'project' ? this.projectItemRules() : this.overrideItemRules();
    return (
      rules.find((rule) => rule.kind === kind && rule.sourceId === sourceId)?.state ??
      this.stateSignal(target, kind)()
    );
  }

  kindState(target: EditorTarget, kind: SourceKind): SourceState {
    return this.stateSignal(target, kind)();
  }
  policyState(policy: SourcePolicy, kind: SourceKind): SourceState {
    return policyDefault(policy.policyV2, kind);
  }

  stateMeaning(state: SourceState): string {
    switch (state) {
      case 'Allow':
        return 'Eligible for the next run.';
      case 'Prioritize':
        return 'Eligible and preferred before ordinary Allow sources.';
      case 'Exclude':
        return 'Not eligible and never materialized.';
    }
  }
    ...
    this.mutationRequest = http
      ...
      .subscribe({
        next: (response) => {
          ...
          this.feedback.set(
            'Project default source policy saved. The Active Scope Summary was refreshed from the server.',
          );
  ...
  runStatusLabel(status: RunStatus): string {
    switch (status) {
      case 'Accepted':
        return 'Execution request was durably accepted.';
      case 'Queued':
        return 'Execution is queued for server materialization.';
      case 'Running':
        return 'Execution is materializing authorized sources.';
      case 'Succeeded':
        return 'Execution succeeded.';
      case 'Failed':
        return 'Execution failed.';
      case 'Stopped':
        return 'Execution was deliberately stopped.';
      case 'Redirected':
        return 'Execution was closed for direction correction.';
    }
  }
  ...
  private buildEditorPolicy(target: EditorTarget): SourcePolicyV2 {
    ...
    const items = [
      ...(target === 'project' ? this.projectItemRules() : this.overrideItemRules()),
  ...
  private compatibilityState(target: EditorTarget, kind: 'Web' | 'ProjectFile'): SourceState {
    ...
    const enabled =
      kind === 'Web'
        ? target === 'project'
          ? this.projectWebEnabled()
          : this.overrideWebEnabled()
        : target === 'project'
          ? this.projectFilesEnabled()
          : this.overrideProjectFilesEnabled();
    if ((state !== 'Exclude') === enabled) {
      return state;
    }
  ...
  private stateSignal(target: EditorTarget, kind: SourceKind) {
    if (target === 'project') {
      switch (kind) {
        case 'Web':
          return this.projectWebState;
        case 'WebSite':
          return this.projectWebSiteState;
        case 'ProjectFile':
          return this.projectFileState;
        case 'ConnectedApp':
          return this.projectConnectedAppState;
      }
    }
    switch (kind) {
      case 'Web':
        return this.overrideWebState;
      case 'WebSite':
        return this.overrideWebSiteState;
      case 'ProjectFile':
        return this.overrideFileState;
      case 'ConnectedApp':
        return this.overrideConnectedAppState;
    }
  }
  ...
  private loadScope(preserveFeedback = false): void {
    ...
    this.readRequest = forkJoin({
      project: http.get<unknown>(`/api/projects/${encodeURIComponent(projectId)}/execution-scope`, {
        withCredentials: true,
      }),
      task: http.get<unknown>(`/api/tasks/${encodeURIComponent(taskId)}/execution-scope`, {
        withCredentials: true,
      }),
    }).subscribe({
      next: (response) => {
  ...
  private applyEditorPolicy(target: EditorTarget, policy: SourcePolicyV2): void {
    ...
    (target === 'project' ? this.projectWebEnabled : this.overrideWebEnabled).set(
      legacyWebEnabled(policy),
    );
    (target === 'project' ? this.projectFilesEnabled : this.overrideProjectFilesEnabled).set(
      legacyProjectFilesEnabled(policy),
    );
  }

  private completeLoadFailure(
    error: unknown,
    generation: number,
    projectId: string,
    taskId: string,
  ...
  private completeMutationFailure(
    error: unknown,
    generation: number,
    projectId: string,
    taskId: string,
...
function mapTaskExecutionScope(value: unknown): TaskExecutionScope {
  ...
  if (
    (origin === 'TaskOverride') !== (overridePolicy !== null) ||
  ...
  return {
    ...
    origin,
    projectDefaultVersion: requiredVersion(
      record['projectDefaultVersion'],
...
function nullableLatestRun(value: unknown): LatestExecutionRun | null {
  ...
  return {
    status,
    majorState:
      record['majorState'] == null
        ? majorStateFromRunStatus(status)
        : requiredMajorState(record['majorState']),
    snapshotScopeOrigin: requiredOrigin(
      record['snapshotScopeOrigin'],
      'Latest execution run source origin',
    ),
    snapshotWebEnabled: requiredBoolean(
      record['snapshotWebEnabled'],
      'Latest execution run Web policy',
    ),
    snapshotProjectFilesEnabled: requiredBoolean(
      record['snapshotProjectFilesEnabled'],
      'Latest execution run file policy',
    ),
    snapshotPolicyV2:
      record['snapshotPolicyV2'] == null
        ? null
        : mapPolicyV2(record['snapshotPolicyV2'], 'Latest execution run source policy'),
  };
}
...
function mapSourcePolicy(value: unknown, label: string): SourcePolicy {
  ...
  const projectFilesEnabled = requiredBoolean(
    record['projectFilesEnabled'],
    `${label} Project-files setting`,
  );
  const policyV2 =
    record['policyV2'] == null
      ? legacyPolicy(webEnabled, projectFilesEnabled)
      : mapPolicyV2(record['policyV2'], `${label} V2`);
  if (
    legacyWebEnabled(policyV2) !== webEnabled ||
    legacyProjectFilesEnabled(policyV2) !== projectFilesEnabled
  ) {
...
function mapPolicyV2(value: unknown, label: string): SourcePolicyV2 {
  ...
  if (!Array.isArray(rawItems) || rawItems.length > 256) {
    throw new Error(`${label} item rules are invalid.`);
  }
  const items = rawItems.map((item, index): SourceRule => {
    ...
    if (typeof sourceId !== 'string' || sourceId.length < 1 || sourceId.length > 256) {
      throw new Error(`${label} source id is invalid.`);
    }
    return {
      kind: requiredSourceKind(rule['kind']),
      sourceId,
      state: requiredSourceState(rule['state']),
    };
  });
  return {
    schemaVersion: 2,
...
function mapInventory(value: unknown): readonly SourceInventoryItem[] {
  ...
  if (!Array.isArray(value) || value.length > 512) {
    throw new Error('Source inventory response is invalid.');
  }
  return value.map((item, index) => {
    const record = requiredRecord(item, `Source inventory ${index}`);
    const sourceId = record['sourceId'];
    const label = record['label'];
    if (typeof sourceId !== 'string' || typeof label !== 'string') {
      throw new Error('Source inventory response is invalid.');
    }
...
function legacyPolicy(webEnabled: boolean, projectFilesEnabled: boolean): SourcePolicyV2 {
  return {
    ...EMPTY_POLICY_V2,
    web: webEnabled ? 'Allow' : 'Exclude',
    projectFile: projectFilesEnabled ? 'Allow' : 'Exclude',
    items: [],
  };
}
function legacyWebEnabled(policy: SourcePolicyV2): boolean {
  return (
    policy.web !== 'Exclude' ||
    policy.items.some((rule) => rule.kind === 'Web' && rule.state !== 'Exclude')
  );
}
function legacyProjectFilesEnabled(policy: SourcePolicyV2): boolean {
  return (
    policy.projectFile !== 'Exclude' ||
    policy.items.some((rule) => rule.kind === 'ProjectFile' && rule.state !== 'Exclude')
  );
}
...
function kindHasEligibleSource(policy: SourcePolicyV2, kind: SourceKind): boolean {
  return (
    policyDefault(policy, kind) !== 'Exclude' ||
...
function ruleRows(rules: readonly SourceRule[], inventory: readonly SourceInventoryItem[]) {
  ...
  for (const item of inventory) {
    ...
    if (keys.add(key)) {
      rows.push({
        ...item,
        configured: rules.some((rule) => `${rule.kind}:${rule.sourceId}` === key),
      });
    }
  }
...
function nullableVersion(value: unknown, label: string): number | null {
  return value == null ? null : requiredVersion(value, label);
}
...
function requiredSourceState(value: unknown): SourceState {
  if (value === 'Allow' || value === 'Prioritize' || value === 'Exclude') {
    return value;
  }
...
function requiredMajorState(value: unknown): MajorState {
  return requiredRunStatus(value);
}
