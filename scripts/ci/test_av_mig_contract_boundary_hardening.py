#!/usr/bin/env python3
"""Regression mutations for SignalR runtime-contract hardening in AV-MIG-02."""
...
class SignalRRuntimeContractMutationTests(unittest.TestCase):
    @classmethod
    ...
    def test_hub_endpoint_allow_anonymous_fails(self) -> None:
        ...
        program.write_text(
            source.replace(");", ").AllowAnonymous();", 1),
            encoding="utf-8",
        )
        with self.assertRaisesRegex(verifier.BoundaryViolation, "must not allow anonymous"):
            verifier.verify_boundary(document, self.policy, root)
