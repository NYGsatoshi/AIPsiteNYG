...
| WPC Project creation/activation | Backend implemented; frontend integration pending | WPC-02A persists canonical Visibility and activation provenance while leaving legacy rows explicitly unknown. WPC-02C implements idempotent, Workspace-scoped `POST /api/workspaces/{workspaceId}/projects`; WPC-02D implements explicit `POST /api/projects/{projectId}/activate` with ProjectGeneral and Task-workflow provisioning under one guarded persistence boundary. The deprecated unscoped `POST /api/projects` remains disabled. The active Angular application does not yet expose the full create/activate flow; Issue #409 owns that frontend integration. See `docs/verification/wpc-final01-integration-audit.md`. |
