...
1. generated OpenAPI 3.1 artifact (`artifacts/openapi/aipportal-openapi.json`) for HTTP path, method, request/response schema and security metadata;
...
The exhaustive current Angular/API binding evidence is maintained in `docs/frontend/api-binding-verification.md`. The migration-blocking P0 operations are additionally pinned in `p0-api-boundary.json` so an accidental route removal is CI-visible.
...
The first-party HTTP client uses the production authentication cookie `.AipPortal.Auth`. The Avalonia transport must use an owned `CookieContainer`/handler abstraction rather than exposing cookies to ViewModels.
