...
Area: Release
Severity: Medium
Description: `docs/archive/plans/FINAL_ACCEPTANCE_TEST_PLAN.md`, `docs/archive/old-specs/SMOKE_TEST.md`, and `docs/API_SMOKE_TESTS.http` define the final pilot acceptance pass, but they still need to be executed against the target environment.
Risk: Documentation could drift from runtime behavior, and a pilot could start with unverified workflows.
Suggested fix: Execute the smoke plan against local dev, Docker Compose, and target on-prem/SaaS-style configuration; record pass/fail evidence in `docs/archive/status/PILOT_STATUS.md`.
Fixed now: no
