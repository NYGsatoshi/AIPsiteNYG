...
export class TaskDetailPageComponent implements OnDestroy {
  ...
  readonly page = computed(() => this.facade.getTaskDetail(this.projectId(), this.taskId()));
  ...
  readonly detailEditing = computed(
    () =>
      ...
      (this.editingChecklistId() !== null &&
        this.editingChecklistText().trim() !== this.originalChecklistText().trim()) ||
      this.commentBody().trim().length > 0 ||
      (this.editingCommentId() !== null &&
        (this.editingCommentBody().trim() !== this.originalCommentBody().trim() ||
          this.editingCommentImportant() !== this.originalCommentImportant())) ||
      this.selectedLabelId().length > 0 ||
      this.newLabelName().trim().length > 0 ||
      (this.editingLabelId() !== null &&
        (this.editingLabelName().trim() !== this.originalLabelName().trim() ||
          this.editingLabelDescription().trim() !== this.originalLabelDescription().trim() ||
          this.editingLabelSortKey() !== this.originalLabelSortKey())) ||
  ...
  constructor() {
    ...
    effect(() =>
      (
        this.facade as unknown as { setTaskBodyEditing?: (editing: boolean) => void }
      ).setTaskBodyEditing?.(this.taskEditorDirty()),
    );
    ...
    this.routeSubscription = this.route.paramMap.subscribe((params) => {
      ...
      if (this.lastRouteKey !== null && this.lastRouteKey !== routeKey) {
  ...
  reloadTaskAfterConflict(): void {
    const taskId = this.taskId();
    if (taskId) {
      this.facade.reloadTaskAfterConflict(taskId);
    }
  }
  ...
  retry(): void {
    const taskId = this.taskId();
    if (taskId) {
      this.facade.retryTaskDetail(taskId);
    }
  }
  retrySection(section: TaskDetailSection): void {
    const taskId = this.taskId();
    if (taskId) {
      this.facade.retrySection(taskId, section);
    }
  }
  /**
  ...
  loadMore(section: 'activity' | 'subtasks' | 'comments' | 'files'): void {
    const taskId = this.taskId();
    if (!taskId) {
      return;
    }
    if (section === 'activity') {
      this.facade.loadMoreActivity(taskId);
    } else if (section === 'subtasks') {
      this.facade.loadMoreSubtasks(taskId);
    } else if (section === 'comments') {
      this.facade.loadMoreComments(taskId);
    } else {
      this.facade.loadMoreFiles(taskId);
    }
  }
  phaseStateLabel(
    category: TaskStageCategory | undefined,
    status: TaskStatus,
  ...
  activityHasConfirmedEmptyState(): boolean {
    return this.sectionState('activity')().status === 'empty';
  }
  createSubtask(): void {
    const vm = this.page();
    const title = this.subtaskTitle().trim();
    if (vm.task && vm.detail?.permissions.canCreateSubtask && title.length <= 300) {
      this.facade.createSubtask(vm.task.id, title, () => this.subtaskTitle.set(''));
    }
  }
  createChecklist(): void {
    const vm = this.page();
    const text = this.checklistText().trim();
    if (vm.task && text && text.length <= 1000 && vm.detail?.permissions.canCreateChecklistItem) {
      this.facade.createChecklist(vm.task.id, text, () => this.checklistText.set(''));
    }
  }
  toggleChecklist(item: { id: string; text: string; isCompleted: boolean; version: string }): void {
    const vm = this.page();
    if (vm.task) {
      this.facade.updateChecklist(vm.task.id, item.id, item.text, !item.isCompleted, item.version);
    }
  }
  editChecklist(item: { id: string; text: string }): void {
    this.editingChecklistId.set(item.id);
    this.editingChecklistText.set(item.text);
    this.originalChecklistText.set(item.text);
  }
  cancelEditChecklist(): void {
    this.editingChecklistId.set(null);
    this.editingChecklistText.set('');
    this.originalChecklistText.set('');
  }
  saveChecklist(item: { id: string; isCompleted: boolean; version: string }): void {
    const vm = this.page();
    const text = this.editingChecklistText().trim();
    if (vm.task && text && text.length <= 1000) {
      this.facade.updateChecklist(vm.task.id, item.id, text, item.isCompleted, item.version, () =>
        this.cancelEditChecklist(),
      );
    }
  }
  deleteChecklist(item: { id: string; version: string }): void {
    const vm = this.page();
    if (vm.task) {
      this.facade.deleteChecklist(vm.task.id, item.id, item.version);
    }
  }
  moveChecklist(itemId: string, direction: -1 | 1): void {
    const vm = this.page();
    if (!vm.task || !vm.detail) {
      return;
    }
    const ids = vm.detail.checklist.map((item) => item.id);
    ...
    this.facade.reorderChecklist(vm.task.id, ids, vm.detail.taskVersion);
  }
  postComment(): void {
    const vm = this.page();
    const body = this.commentBody().trim();
    if (vm.task && body && body.length <= 12000 && vm.detail?.permissions.canCreateComment) {
      this.facade.createComment(vm.task.id, body, this.commentImportant(), () => {
        this.commentBody.set('');
        this.commentImportant.set(false);
      });
    }
  }
  editComment(comment: { id: string; body: string | null; isImportant: boolean }): void {
    if (comment.body !== null) {
      this.editingCommentId.set(comment.id);
      this.editingCommentBody.set(comment.body);
      this.editingCommentImportant.set(comment.isImportant);
      this.originalCommentBody.set(comment.body);
      this.originalCommentImportant.set(comment.isImportant);
    }
  }
  cancelEditComment(): void {
    this.editingCommentId.set(null);
    this.editingCommentBody.set('');
    this.editingCommentImportant.set(false);
    this.originalCommentBody.set('');
    this.originalCommentImportant.set(false);
  }
  saveComment(comment: { id: string; version: string }): void {
    const vm = this.page();
    const body = this.editingCommentBody().trim();
    if (vm.task && body && body.length <= 12000) {
      this.facade.updateComment(
        vm.task.id,
        comment.id,
        body,
        this.editingCommentImportant(),
        comment.version,
        () => this.cancelEditComment(),
      );
    }
  }
  toggleImportant(comment: {
    id: string;
    body: string | null;
    isImportant: boolean;
    version: string;
  ...
  deleteComment(comment: { id: string; version: string }): void {
    const vm = this.page();
    if (vm.task) {
      this.facade.deleteComment(vm.task.id, comment.id, comment.version);
    }
  }
  toggleWatch(): void {
    const vm = this.page();
    if (vm.task && vm.detail?.permissions.canChangeWatch) {
      this.facade.setWatch(
        vm.task.id,
        !vm.detail.watchState.isWatching,
        vm.detail.watchState.version,
      );
    }
  }
  removeLabel(labelId: string): void {
    const vm = this.page();
    if (vm.task && vm.detail) {
      this.facade.removeLabel(vm.task.id, labelId, vm.detail.taskVersion);
    }
  }
  applyLabel(): void {
    const vm = this.page();
    const labelId = this.selectedLabelId();
    if (vm.task && labelId && vm.detail?.permissions.canApplyLabels) {
      this.facade.applyLabel(vm.task.id, labelId, vm.detail.taskVersion, () =>
        this.selectedLabelId.set(''),
      );
    }
  }
  refreshLabelDefinitions(): void {
    const vm = this.page();
    if (vm.task) {
      this.facade.loadProjectLabelDefinitions(vm.task.projectId, true);
    }
  }
  createLabel(): void {
    const vm = this.page();
    if (
      vm.task &&
      this.newLabelName().trim().length > 0 &&
      this.newLabelName().trim().length <= TASK_LABEL_NAME_MAX_LENGTH &&
      vm.detail?.permissions.canManageLabelDefinitions
    ) {
      this.facade.createProjectLabel(vm.task.id, vm.task.projectId, this.newLabelName(), () =>
  ...
  editLabel(label: {
    ...
    this.editingLabelDescription.set(label.description ?? '');
    this.editingLabelSortKey.set(label.sortKey);
    this.originalLabelName.set(label.name);
    this.originalLabelDescription.set(label.description ?? '');
    this.originalLabelSortKey.set(label.sortKey);
  }
  cancelEditLabel(): void {
    this.editingLabelId.set(null);
    this.editingLabelName.set('');
    this.editingLabelDescription.set('');
    this.editingLabelSortKey.set('');
    this.originalLabelName.set('');
    this.originalLabelDescription.set('');
    this.originalLabelSortKey.set('');
  }
  saveLabel(label: { id: string; version: string }): void {
    const vm = this.page();
    const sortKey = Number(this.editingLabelSortKey());
    if (
      vm.task &&
      this.editingLabelName().trim() &&
      this.editingLabelName().trim().length <= TASK_LABEL_NAME_MAX_LENGTH &&
      this.editingLabelDescription().trim().length <= TASK_LABEL_DESCRIPTION_MAX_LENGTH &&
      Number.isSafeInteger(sortKey) &&
      sortKey >= 0
    ) {
      this.facade.updateProjectLabel(
        vm.task.id,
        vm.task.projectId,
        label.id,
        this.editingLabelName(),
        this.editingLabelDescription(),
        this.editingLabelSortKey(),
        label.version,
        () => this.cancelEditLabel(),
      );
    }
  }
  setLabelArchived(label: { id: string; version: string }, archived: boolean): void {
    const vm = this.page();
    if (vm.task) {
      this.facade.setProjectLabelArchived(
        vm.task.id,
        vm.task.projectId,
        label.id,
        label.version,
  ...
  checklistCompletedCount(): number {
    return this.page().detail?.checklist.filter((item) => item.isCompleted).length ?? 0;
  }
  isAppliedLabel(): boolean {
    const detail = this.page().detail;
    return !!detail?.labels.some((label) => label.id === this.selectedLabelId());
  }
  removeFile(associationId: string): void {
    const vm = this.page();
    if (vm.task && vm.detail) {
      this.facade.removeFile(vm.task.id, associationId, vm.detail.taskVersion);
    }
  }
  setSelectedAttachment(id: string | null): void {
    this.selectedAttachmentId.set(id ?? '');
  }
  associateSelectedFile(): void {
    const vm = this.page();
    const attachmentId = this.selectedAttachmentId();
    if (vm.task && vm.detail && attachmentId) {
      this.facade.associateFile(vm.task.id, attachmentId, vm.detail.taskVersion, () =>
        this.selectedAttachmentId.set(''),
      );
    }
  }
  loadPickerFiles(): void {
    const vm = this.page();
    if (vm.detail?.permissions.canAssociateFiles && vm.detail.workspaceId) {
      this.files.loadPickerFilesForWorkspace(vm.detail.workspaceId);
    }
  }
  downloadFile(attachmentId: string, fileObjectId: string, fileName: string): void {
    const taskId = this.taskId();
    ...
    this.files.downloadAttachment(attachmentId, fileName, {
      workspaceId,
      fileObjectId,
      isCurrent: () => this.taskId() === taskId && this.projectId() === projectId,
      onState: (_, message) => this.fileDownloadMessage.set(message),
      onPermissionDenied: () => this.facade.retryTaskDetail(taskId),
    });
  }
  loadMorePickerFiles(): void {
    this.files.loadMorePickerFiles();
  }
  retryPickerFiles(): void {
    this.files.retryPickerFiles();
  }

  taskFileMetadataLabel(file: {
  ...
  taskFileScanLabel(scanStatus: string): string {
    return this.i18n.translate('files.task.scan', {
      status: this.i18n.taskFileScanStatusLabel(scanStatus),
    });
  }

  taskFileAccessLabel(accessState: string): string {
    return this.i18n.translate('files.task.access', {
      status: this.i18n.taskFileAccessStateLabel(accessState),
    });
  }
  ...
  taskFileSectionMessage(state: TaskDetailSectionState): string {
    if (this.i18n.locale() === 'en') {
      return state.message ?? this.i18n.translate('files.task.error');
    }
    if (state.status === 'permissionDenied') {
      return this.i18n.translate('api.permissionDenied');
    }
    if (state.status === 'conflict') {
      return this.i18n.translate('api.conflict');
    }
  ...
  private resetLocalTaskDraftState(): void {
    ...
    this.subtaskTitle.set('');
    this.checklistText.set('');
    this.editingChecklistId.set(null);
    this.editingChecklistText.set('');
    this.originalChecklistText.set('');
    this.taskEditorDirty.set(false);
    this.researchPlanDirty.set(false);
    this.commentBody.set('');
    this.commentImportant.set(false);
    this.cancelEditComment();
    this.selectedLabelId.set('');
    ...
    this.cancelEditLabel();
    this.selectedAttachmentId.set('');
    this.fileDownloadMessage.set('');
    this.files.cancelAttachmentDownloads();
    this.files.clearPickerFiles();
    this.facade.setDetailEditing(false);
  }
}
