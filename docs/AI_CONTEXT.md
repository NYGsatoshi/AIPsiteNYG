...
| WPC Project creation/activation | Blocked foundation | Project responses preserve nullable `GroupId` and expose `VersionNo`. Generic `Planning -> Active`, ambiguous `Suspended -> Active`, and Archived/Deleted recovery without provenance return a non-mutating 409; an already operational Project retains metadata-only Active updates and ordinary `Active -> Review -> Active`. No lifecycle-provenance persistence was guessed. Canonical Visibility/backfill, Workspace-scoped create, optional Group API binding, Project create idempotency, authoritative create capability, canonical default Channel provisioning, and atomic activation remain unresolved. See `docs/verification/wpc-01-workspace-project-creation-foundation.md`. |
...
| Search | Partially implemented | Project-derived results now use the same current SQL-translatable Project read boundary as detail/list for Project, Task, Artifact, ActivityLog, Comment, and project-bound Message results. Canonical Visibility persistence is unresolved and `/search` UI remains unavailable. |
...
- Critical backend logic defects still affect scoped announcements,
  conversation persistence, and message attachments. PR #281 closes the
