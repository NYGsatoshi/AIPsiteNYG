...
| Announcements | #378 durable announcement delivery candidate; #382 local preview remains separate | The server-authorized audience projection returns only an authorized scope, display name, and estimated recipient count; it does not disclose recipient identities. The production Angular editor has distinct Save draft, local Preview, and one-shot confirmation actions. Its server-owned `AnnouncementDraft` lifecycle is `Draft -> Scheduled -> Published`: both an immediate Publish and a user-scheduled confirmation first persist Scheduled work, and only the bounded PostgreSQL-coordinated worker may create the normal Announcement after due-time author/audience reauthorization. Local IANA wall-clock scheduling is resolved once to UTC server-side; create and delivery commands use Tenant/actor/operation idempotency plus optimistic versioning. Browser UI never invents a published item or delivery completion. Draft read/mutation is current-author/current-target reauthorized and cross-scope denial is redacted. CTA/link, attachments, cohorts, analytics, recurring scheduling, and cancellation remain excluded (#382/#383/#387 are not absorbed). See `docs/verification/p0-announcement-publish-confirmation.md` and `docs/verification/p1-announcement-local-preview.md`. |
| Structured Artifact Report reader | Issue #373 implementation candidate | An immutable v1 `ArtifactReportDocument` binds bounded plain-text Sections and server-validated UTF-16 Citation anchors to exact per-version Claims. The normal Project/optional-Task-bound endpoint returns server-split render runs and only currently reauthorized Evidence; inaccessible Evidence is omitted without a hidden count. Angular Project and Task Report routes preserve document scroll while a focus-safe desktop Inspector or narrow-screen full view presents the selected citation evidence. See `docs/verification/p0-report-evidence-inspector.md`. |
...
| Local filesystem files | Partially implemented; Issues #341, #345, and #356 verified | Authorization, policy, repository, and storage exist. The Workspace inventory is a bounded server-authorized list and projects a per-row `canDelete` presentation capability without per-row authorization queries. The Files search surface uses the same `/api/search` Workspace File boundary for filename plus Type, Modified, and current-uploader facets; backend Search owns filtered membership/count, while a strict browser adapter rejects mismatched File/Workspace records and never consumes snippets or storage paths. Applied facets remain visible as removable accessible chips, and protected query/results clear with Workspace or authorization state. The Angular Files surface replaces its normal toolbar with selected-row actions, confirms destructive targets by the authorized server-projected display label/count, and processes multi-selection through the existing single-file DELETE route serially with explicit partial/non-atomic reporting. Search rows never infer delete authority, and every DELETE or preview/download operation retains its independent server reauthorization/grant boundary. One responsive inspector switches between grant-backed Preview, staged Details, and an explicit unavailable Activity state with accessible tab semantics; it renders only authorized list/search fields, exposes no edit control without a server contract, and makes no broad Audit/activity/version query. File-specific activity/version history remains for Issue #363. Rename, move, share, public links, folder search, broader uploader enumeration, and an atomic batch-delete contract remain absent. Upload/database failure cleanup and controlled missing-file handling are still incomplete. See `docs/verification/p0-files-search-filter-chips.md`, `docs/verification/p0-files-contextual-actions.md`, and `docs/verification/p0-files-inspector-tabs.md`. |
