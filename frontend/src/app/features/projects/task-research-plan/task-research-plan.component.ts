...
export class TaskResearchPlanComponent implements OnChanges, OnDestroy {
  ...
  readonly dirty = computed(
    () => JSON.stringify(this.draft()) !== JSON.stringify(this.loadedDraft()),
  );
  ...
  updateText(index: number, field: 'title' | 'objective' | 'scopeSummary', value: string): void {
    ...
    this.invalidateReview();
    this.draft.update((steps) =>
      steps.map((step, candidate) => (candidate === index ? { ...step, [field]: value } : step)),
    );
  }

  updateStatus(index: number, value: string): void {
    if (!this.canManage() || this.busy() || !this.statuses.includes(value as StepStatus)) {
      return;
    }
    this.invalidateReview();
    this.draft.update((steps) =>
      steps.map((step, candidate) =>
        candidate === index ? { ...step, status: value as StepStatus } : step,
      ),
    );
  }
      ...
      .subscribe({
        next: (response) => {
          ...
          if (mapped?.baseVersion !== current.version) {
            this.error.set(
              'The Research Plan change preview could not be read. Reload and try again.',
            );
            return;
          }
    ...
    this.saveRequest = http
      ...
      .subscribe({
        next: (response) => {
          ...
          this.applyPlan(plan);
          this.feedback.set(
            `Research Plan revision ${plan.currentRevision?.number ?? ''} saved from the reviewed diff.`.trim(),
          );
        },
        error: (error) => {
          if (requestGeneration !== this.generation()) {
  ...
  private load(): void {
    ...
    this.loadRequest = http
      .get<unknown>(`/api/tasks/${encodeURIComponent(taskId)}/research-plan`, {
        withCredentials: true,
      })
      .subscribe({
        next: (response) => {
        ...
        error: (error) => {
          ...
          this.loadedDraft.set([]);
          this.error.set(
            normalizeApiError(error).message || 'The Research Plan could not be loaded.',
          );
          this.loading.set(false);
        },
        complete: () => {
...
function mapResearchPlan(value: unknown): ResearchPlan | null {
  ...
  if (value['currentRevision'] !== null && revision === null) {
    return null;
  }
  return {
...
function mapProposedStep(value: unknown): ProposedStep | null {
  ...
  if (rawBaseStepId !== null && typeof rawBaseStepId !== 'string') {
    return null;
  }
...
function mapDiff(value: unknown): StepDiff | null {
  ...
  if (rawBeforePosition !== null && typeof rawBeforePosition !== 'number') {
    return null;
  }
  if (rawAfterPosition !== null && typeof rawAfterPosition !== 'number') {
    return null;
  }
  const baseStepId: string | null = typeof rawBaseStepId === 'string' ? rawBaseStepId : null;
  const beforePosition: number | null =
    typeof rawBeforePosition === 'number' ? rawBeforePosition : null;
  const afterPosition: number | null =
    typeof rawAfterPosition === 'number' ? rawAfterPosition : null;
  const before = value['before'] === null ? null : mapStep(value['before']);
  const after = value['after'] === null ? null : mapProposedStep(value['after']);
  if (value['before'] !== null && before === null) {
    return null;
  }
  if (value['after'] !== null && after === null) {
    return null;
  }
  return {
    kinds: value['kinds'],
...
function mapImpact(value: unknown): ImpactSummary | null {
  if (
    !isRecord(value) ||
    typeof value['beforeStepCount'] !== 'number' ||
    typeof value['afterStepCount'] !== 'number' ||
    typeof value['executionStepCountChanged'] !== 'boolean' ||
    typeof value['executionOrderChanged'] !== 'boolean' ||
    typeof value['sourceScopeGuidanceChanged'] !== 'boolean' ||
    typeof value['deliverableAlignmentReviewRequired'] !== 'boolean' ||
  ...
  const items = value['items'].map((item) => {
    ...
    if (rawStepPosition !== null && typeof rawStepPosition !== 'number') {
      return null;
    }
    const stepPosition: number | null =
      typeof rawStepPosition === 'number' ? rawStepPosition : null;
    return { kind: item['kind'], message: item['message'], stepPosition } satisfies ImpactItem;
  });
  if (!items.every((item): item is ImpactItem => item !== null)) {
    return null;
  }
  return {
...
function mapPreview(value: unknown): PlanPreview | null {
  if (
    ...
    !Array.isArray(value['proposedSteps']) ||
  ...
  if (rawBaseRevisionId !== null && typeof rawBaseRevisionId !== 'string') {
    return null;
  }
  if (rawBaseRevisionNumber !== null && typeof rawBaseRevisionNumber !== 'number') {
    return null;
  }
  const baseRevisionId: string | null =
    typeof rawBaseRevisionId === 'string' ? rawBaseRevisionId : null;
  const baseRevisionNumber: number | null =
    typeof rawBaseRevisionNumber === 'number' ? rawBaseRevisionNumber : null;
  ...
  if (
    !proposedSteps.every((step): step is ProposedStep => step !== null) ||
    !changes.every((change): change is StepDiff => change !== null) ||
    !impact
  ) {
    return null;
  }
