...
| Admin audit UI | Partially implemented; Issue #386 accessibility hardening candidate | The Angular route consumes only the server-authorized, redacted `AuditGridRowResponse` list/detail projection. The candidate uses readable column headers, text-bearing severity/result badges with a neutral fixed fallback for unknown wire values, a fixed local status region, 24px row actions, and keyboard/focus-safe non-modal detail inspection at desktop and 320px. It neither exposes raw metadata nor treats the current page length as a server total. The retained AG grid has focused axe coverage; the disabled-by-default Syncfusion adapter has an isolated rendered keyboard/header/target smoke. Audit Review save, server-backed audit filters, exports, realtime, saved views, and any additional/raw detail fields remain separate backend contracts. See `docs/verification/p1-audit-ui-accessibility.md`. |
